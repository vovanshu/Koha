...
sub fixup_cardnumber {
    ...
    my $ILMN = C4::Context->preference("IndependentLibraryMemberNum");

    if ( $ILMN ) {

        $max = Koha::Patrons->search()->search(
        {
            branchcode => {-like => $self->branchcode.'%'}
        }, {
...
sub generate_userid {
    ...
    my $userid = '';
    my $genuseridbcn = C4::Context->preference('generate_userid_bycardnumber');
